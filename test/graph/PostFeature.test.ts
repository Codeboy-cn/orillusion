...
// C8 acceptance: PostFeature registered at stage=Post, reads _ColorBuffer,
// writes _FinalColor. After one frame (FXAAPost runs) the _FinalColor
// handle resolves to a non-null texture. PostFeature is registered
...
await test('PostFeature registered eagerly at stage=Post, reads _ColorBuffer, writes _FinalColor', async () => {
    ...
    const feature = view.renderGraph!.getFeature('PostFeature') as PostFeature | null
    if (!feature) throw new Error('PostFeature not registered')
