...
// produces a ForwardRendererJob, `view.renderGraph` is a live
// RenderGraph, and the engine renders cleanly through the graph +
// legacy fallback for un-migrated stages.

await test('useFrameGraph=true produces a ForwardRendererJob with a live RenderGraph', async () => {
    ...
    // at least ClusterLightingPass; later C* PRs append more.
...
await test('Phase D: default (no override) yields a ForwardRendererJob', async () => {
