...
// C5 acceptance: ReflectionFeature registered at stage GI, exposes
...
await test('ReflectionFeature is registered at stage=GI and exposes _ReflectionCubeMap', async () => {
    ...
    const feature = view.renderGraph!.getFeature('ReflectionFeature') as ReflectionFeature | null
    if (!feature) throw new Error('ReflectionFeature not registered')
