...
/**
 * Assert a continued-SRT record carries no motion. The record is not uniformly
 * zero when idle: slots 3..6 hold a rotation delta *quaternion*, whose no-op
 * value is identity (0, 0, 0, 1), so only scale (0..2) and translation (7..9)
 * are compared against zero.
 */
function expectIdleContinuedSRT(matIndex: number) {
    const base = matIndex * WasmMatrix.continuedSrtStride;
    const buf = WasmMatrix.matrixContinuedSRTBuffer;
    for (let i of [0, 1, 2, 7, 8, 9]) {
        expect(buf[base + i]).toEqual(0);
    }
    expect(buf[base + 3]).toEqual(0);
    expect(buf[base + 4]).toEqual(0);
    expect(buf[base + 5]).toEqual(0);
    // identity quaternion, or a pristine all-zero slot never written to
    expect(buf[base + 6] === 1 || buf[base + 6] === 0).toEqual(true);
}
