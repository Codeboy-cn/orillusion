 ...
 * @group Graph
 */
export class GPUCullFeature extends RenderFeature {
    public readonly name = 'GPUCullFeature';
    public readonly stage = RenderStage.BeforeShadows;
    public readonly reads: readonly string[] = [];
    public readonly writes = [VISIBILITY_BUFFER, DRAW_CMDS_BUFFER, DRAW_COUNT_BUFFER];

    private readonly _system: GPUCullSystem;
    ...
    constructor(ctx: Context3D) {
        super();
        this._system = new GPUCullSystem(ctx);
    }

    public registerResources(pool: { registerExternal<T>(name: string, getter: () => T): void }): void {
        pool.registerExternal<unknown>(VISIBILITY_BUFFER, () => this._system.visibilityBuffer);
        pool.registerExternal<unknown>(DRAW_CMDS_BUFFER, () => this._system.drawCmdsBuffer);
        pool.registerExternal<unknown>(DRAW_COUNT_BUFFER, () => this._system.drawCountBuffer);
    }
