...
export class Preprocessor {
    ...
    protected static parsePreprocess(context: PreprocessorContext, code: string, defineValue: { [name: string]: any }): string {
        ...
        // Order matters and is pinned with locals rather than left to operand
        // evaluation order: parsePreprocessCommand populates defineValue as it
        // walks `#define` lines, so the header (which precedes every directive)
        // must expand before that call, and the tail (which follows them all)
        // after it — matching C-preprocessor scoping.
        let head = expand(header, defineValue);
        let body = this.parsePreprocessCommand(context, codeBlock, defineValue);
        return head + body + expand(tail, defineValue);
    }
    ...
    protected static parseCondition(condition: string, defineValue: { [name: string]: any }): boolean {
        try {
            ...
            // Single-pass expand avoids infinite loops on self-reference.
