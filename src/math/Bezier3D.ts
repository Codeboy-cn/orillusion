...
export class Bezier3D {
    ...
    public static calculateCubicBezierPoint(t: number, p0: Vector3, c1: Vector3, c2: Vector3, p3: Vector3): Vector3 {
        ...
        let tt = t * t;
        let ttt = t * t * t;
        let p = p0.clone().mul(uuu);
        let tp1 = c1.clone().mul(3);
        tp1 = tp1.mul(t);
        tp1 = tp1.mul(uu);

        let tp2 = c2.clone().mul(3);
        tp2 = tp2.mul(tt);
        tp2 = tp2.mul(u);

        let tp3 = p3.clone().mul(ttt);
        p = p.add(tp1);
        p = p.add(tp2);
        p = p.add(tp3);
        return p;
    ...
    public static bezierPoint(t: number, p0: Vector3, c1: Vector3, p1: Vector3): Vector3 {
        ...
        let pp0 = p0.clone().mul(uu);

        let cc1 = c1.clone().mul(2);
        cc1.scaleBy(u);
        cc1.scaleBy(t);

        let pp1 = p1.clone().mul(tt);
    ...
    private static calculateCubicBezierPoints(t: number, ps: Vector3[], skip: number): Vector3 {
        ...
        let tt = t * t;
        let ttt = t * t * t;
        let p = ps[skip].clone().mul(uuu);
        let tp1 = ps[skip + 1].clone().mul(3);
        tp1 = tp1.mul(t);
        tp1 = tp1.mul(uu);

        let tp2 = ps[skip + 2].clone().mul(3);
        tp2 = tp2.mul(tt);
        tp2 = tp2.mul(u);

        let tp3 = ps[skip + 3].clone().mul(ttt);
        p = p.add(tp1);
        p = p.add(tp2);
        p = p.add(tp3);
